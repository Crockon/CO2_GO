...
#define FLOW_THRESHOLD_SLPM    0.005f
...
bool prev_status = 0;
bool lineBlocked = 0;
bool wasLineBlocked = false; // <-- ADD THIS LINE
...
//__________________________________
//Apnea alarm setup
enum AlarmState { ALARM_OFF, ALARM_APNEA, ALARM_BLOCKED };
...
void alarmCount(float co2mmhg) {
  ...
  // 2. CLEAR THE WARNING IMMEDIATEY WHEN UNBLOCKED
  if (!lineBlocked && wasLineBlocked) {
...
void updateGraph(int co2val) {
  ...
  char text[32];
  sprintf(text, "F: %.2f", avg);
  my_lcd.drawString(text, 380, 240);
